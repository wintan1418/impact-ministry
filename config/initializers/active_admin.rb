ActiveAdmin.setup do |config|
  ...
  #
  config.site_title = "Impact Ministry"
  ...
  #
  # IMPACT Ministry: integrate with Rails 8 native auth + our User#role enum.
  # `authenticate_admin!` is defined in ApplicationController and ensures
  # current_user is signed in AND has role editor or admin.
  ...
  #
  # IMPACT Ministry: current_user is set by our Authentication concern.
  ...
  #
  # IMPACT Ministry: logout via Rails 8 native sessions (DELETE /session).
  # ActiveAdmin master only exposes `logout_link_path`; we render the link with
  # `data-turbo-method="delete"` via a header override (app/views/active_admin/...).
  config.logout_link_path = :session_path

  # If an editor lands somewhere only admins can see, redirect to dashboard with a flash.
  config.on_unauthorized_access = :access_denied
